...
if __name__ ==  '__main__':
    ...
    #Saving spectrum
    if not os.path.exists('./Results/Spectrum-' + str(N) + 'x' + str(S) + '.txt'):
        np.savetxt('./Results/Spectrum-' + str(N) + 'x' + str(S) + '.txt', E, header = 'Runtime: ' + str(time.time() - start))
    else:
        i = 1
        while True:
            if not os.path.exists('./Results/Spectrum-' + str(N) + 'x' + str(S) + '(' + str(i) + ')' + '.txt'):
                np.savetxt('./Results/Spectrum-' + str(N) + 'x' + str(S) + '(' + str(i) + ')' + '.txt', E, header = 'Runtime: ' + str(time.time() - start))
                break
            i += 1
